...
public record VanillaChargedCreeperExplosionLoot(HolderLookup.Provider registries) implements LootTableSubProvider
{
    ...
    public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> biConsumer) {
        ...
        ArrayList<ConditionUserBuilder> arrayList = new ArrayList<ConditionUserBuilder>(ENTRIES.size());
        for (Entry entry : ENTRIES) {
            ...
            arrayList.add(NestedLootTable.lootTableReference(entry.lootTable).when(builder));
        }
        biConsumer.accept(BuiltInLootTables.CHARGED_CREEPER, LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0f)).add(AlternativesEntry.alternatives((LootPoolEntryContainer.Builder[])arrayList.toArray(LootPoolEntryContainer.Builder[]::new)))));
    }
