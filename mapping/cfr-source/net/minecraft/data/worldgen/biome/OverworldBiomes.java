...
public class OverworldBiomes {
    ...
    public static int calculateSkyColor(float f) {
        ...
        return ARGB.opaque(Mth.hsvToRgb(0.62222224f - f2 * 0.05f, 0.5f + f2 * 0.1f, 1.0f));
    }

    private static Biome.BiomeBuilder baseBiome(float f, float f2) {
        return new Biome.BiomeBuilder().hasPrecipitation(true).temperature(f).downfall(f2).setAttribute(EnvironmentAttributes.SKY_COLOR, OverworldBiomes.calculateSkyColor(f)).specialEffects(new BiomeSpecialEffects.Builder().waterColor(4159204).build());
    }
    ...
    public static Biome taiga(HolderGetter<PlacedFeature> holderGetter, HolderGetter<ConfiguredWorldCarver<?>> holderGetter2, boolean bl) {
        ...
        int n = bl ? 4020182 : 4159204;
        return OverworldBiomes.baseBiome(bl ? -0.5f : 0.25f, bl ? 0.4f : 0.8f).specialEffects(new BiomeSpecialEffects.Builder().waterColor(n).build()).mobSpawnSettings(builder.build()).generationSettings(builder2.build()).build();
    }
    ...
    public static Biome beach(HolderGetter<PlacedFeature> holderGetter, HolderGetter<ConfiguredWorldCarver<?>> holderGetter2, boolean bl, boolean bl2) {
        ...
        int n = bl ? 4020182 : 4159204;
        return OverworldBiomes.baseBiome(f, bl3 ? 0.4f : 0.3f).specialEffects(new BiomeSpecialEffects.Builder().waterColor(n).build()).mobSpawnSettings(builder.build()).generationSettings(builder2.build()).build();
    }
