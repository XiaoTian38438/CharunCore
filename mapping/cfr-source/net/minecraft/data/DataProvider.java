...
public interface DataProvider {
    ...
    public static <T, E> CompletableFuture<?> saveAll(CachedOutput cachedOutput, Codec<E> codec, Function<T, Path> function, Map<T, E> map) {
        return DataProvider.saveAll(cachedOutput, (E object) -> codec.encodeStart(JsonOps.INSTANCE, object).getOrThrow(), function, map);
    }
    ...
    public static <T> CompletableFuture<?> saveStable(CachedOutput cachedOutput, Codec<T> codec, T t, Path path) {
        return DataProvider.saveStable(cachedOutput, JsonOps.INSTANCE, codec, t, path);
    }
    ...
    private static <T> CompletableFuture<?> saveStable(CachedOutput cachedOutput, DynamicOps<JsonElement> dynamicOps, Codec<T> codec, T t, Path path) {
        JsonElement jsonElement = codec.encodeStart(dynamicOps, (JsonElement)t).getOrThrow();
        return DataProvider.saveStable(cachedOutput, jsonElement, path);
    }
