...
implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cachedOutput) {
        return this.registries.thenCompose(provider -> {
            ...
            return CompletableFuture.allOf((CompletableFuture[])RegistryDataLoader.WORLDGEN_REGISTRIES.stream().flatMap(registryData -> this.dumpRegistryCap(cachedOutput, (HolderLookup.Provider)provider, (DynamicOps<JsonElement>)registryOps, (RegistryDataLoader.RegistryData)registryData).stream()).toArray(CompletableFuture[]::new));
        });
    }
    ...
    private static <E> CompletableFuture<?> dumpValue(Path path, CachedOutput cachedOutput, DynamicOps<JsonElement> dynamicOps, Encoder<E> encoder, E e) {
        return encoder.encodeStart(dynamicOps, e).mapOrElse(jsonElement -> DataProvider.saveStable(cachedOutput, jsonElement, path), error -> CompletableFuture.failedFuture(new IllegalStateException("Couldn't generate file '" + String.valueOf(path) + "': " + error.message())));
    }
