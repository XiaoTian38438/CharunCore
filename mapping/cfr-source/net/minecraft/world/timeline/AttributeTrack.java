...
public record AttributeTrack<Value, Argument>(AttributeModifier<Value, Argument> modifier, KeyframeTrack<Argument> argumentTrack) {
    ...
    public static DataResult<AttributeTrack<?, ?>> validatePeriod(AttributeTrack<?, ?> attributeTrack, int n) {
        return KeyframeTrack.validatePeriod(attributeTrack.argumentTrack(), n).map(keyframeTrack -> attributeTrack);
    }
}
