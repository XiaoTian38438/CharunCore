...
public class Timeline {
    ...
    public static final Codec<Timeline> DIRECT_CODEC = RecordCodecBuilder.create(instance -> instance.group(ExtraCodecs.POSITIVE_INT.optionalFieldOf("period_ticks").forGetter(timeline -> timeline.periodTicks), TRACKS_CODEC.optionalFieldOf("tracks", Map.of()).forGetter(timeline -> timeline.tracks)).apply((Applicative<Timeline, ?>)instance, Timeline::new)).validate(Timeline::validateInternal);
    ...
    private static DataResult<Timeline> validateInternal(Timeline timeline2) {
        if (timeline2.periodTicks.isEmpty()) {
            return DataResult.success(timeline2);
        }
        int n = timeline2.periodTicks.get();
        DataResult<Timeline> dataResult = DataResult.success(timeline2);
        for (AttributeTrack<?, ?> attributeTrack2 : timeline2.tracks.values()) {
            dataResult = dataResult.apply2stable((timeline, attributeTrack) -> timeline, AttributeTrack.validatePeriod(attributeTrack2, n));
        }
        return dataResult;
    }
    ...
    public static class Builder {
        ...
        public Builder setPeriodTicks(int n) {
            this.periodTicks = Optional.of(n);
            return this;
        }
