...
public record SavedTick<T>(T type, BlockPos pos, int delay, TickPriority priority) {
    ...
    public static <T> Codec<SavedTick<T>> codec(Codec<T> codec) {
        ...
        return RecordCodecBuilder.create(instance -> instance.group(((MapCodec)codec.fieldOf("i")).forGetter(SavedTick::type), mapCodec.forGetter(SavedTick::pos), ((MapCodec)Codec.INT.fieldOf("t")).forGetter(SavedTick::delay), ((MapCodec)TickPriority.CODEC.fieldOf("p")).forGetter(SavedTick::priority)).apply((Applicative<SavedTick, ?>)instance, SavedTick::new));
    }
