...
public final class DensityFunctions {
    ...
    public static DensityFunction findTopSurface(DensityFunction densityFunction, DensityFunction densityFunction2, int n, int n2) {
        return new FindTopSurface(densityFunction, densityFunction2, n, n2);
    }
    ...
    protected record Mapped(Type type, DensityFunction input, double minValue, double maxValue) implements PureTransformer
    {
        public static Mapped create(Type type, DensityFunction densityFunction) {
            ...
            double d3 = Mapped.transform(type, d);
            double d4 = Mapped.transform(type, d2);
            if (type == Type.INVERT) {
                if (d < 0.0 && d2 > 0.0) {
                    return new Mapped(type, densityFunction, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
                }
                return new Mapped(type, densityFunction, d4, d3);
            }
            if (type == Type.ABS || type == Type.SQUARE) {
                return new Mapped(type, densityFunction, Math.max(0.0, d), Math.max(d3, d4));
            }
            return new Mapped(type, densityFunction, d3, d4);
        }
    ...
    {
        private static final MapCodec<FindTopSurface> DATA_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)DensityFunction.HOLDER_HELPER_CODEC.fieldOf("density")).forGetter(FindTopSurface::density), ((MapCodec)DensityFunction.HOLDER_HELPER_CODEC.fieldOf("upper_bound")).forGetter(FindTopSurface::upperBound), ((MapCodec)Codec.intRange(DimensionType.MIN_Y * 2, DimensionType.MAX_Y * 2).fieldOf("lower_bound")).forGetter(FindTopSurface::lowerBound), ((MapCodec)ExtraCodecs.POSITIVE_INT.fieldOf("cell_height")).forGetter(FindTopSurface::cellHeight)).apply((Applicative<FindTopSurface, ?>)instance, FindTopSurface::new));
        public static final KeyDispatchDataCodec<FindTopSurface> CODEC = DensityFunctions.makeCodec(DATA_CODEC);

        @Override
        public double compute(DensityFunction.FunctionContext functionContext) {
            int n = Mth.floor(this.upperBound.compute(functionContext) / (double)this.cellHeight) * this.cellHeight;
            if (n <= this.lowerBound) {
                return this.lowerBound;
            }
            for (int i = n; i >= this.lowerBound; i -= this.cellHeight) {
                DensityFunction.SinglePointContext singlePointContext = new DensityFunction.SinglePointContext(functionContext.blockX(), i, functionContext.blockZ());
