...
public class PoolAliasBindings {
    ...
    public static void registerTargetsAsPools(BootstrapContext<StructureTemplatePool> bootstrapContext, Holder<StructureTemplatePool> holder, List<PoolAliasBinding> list) {
        list.stream().flatMap(PoolAliasBinding::allTargets).map(resourceKey -> resourceKey.identifier().getPath()).forEach(string -> Pools.register(bootstrapContext, string, new StructureTemplatePool(holder, List.of(Pair.of(StructurePoolElement.single(string), 1)), StructureTemplatePool.Projection.RIGID)));
    }
}
