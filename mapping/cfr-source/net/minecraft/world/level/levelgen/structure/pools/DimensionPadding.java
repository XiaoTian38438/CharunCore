...
public record DimensionPadding(int bottom, int top) {
    ...
    public static final Codec<DimensionPadding> CODEC = Codec.either(ExtraCodecs.NON_NEGATIVE_INT, RECORD_CODEC).xmap(either -> either.map(DimensionPadding::new, Function.identity()), dimensionPadding -> dimensionPadding.hasEqualTopAndBottom() ? Either.left(dimensionPadding.bottom) : Either.right(dimensionPadding));
