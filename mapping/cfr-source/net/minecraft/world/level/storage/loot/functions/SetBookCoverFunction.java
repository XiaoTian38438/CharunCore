...
extends LootItemConditionalFunction {
    public static final MapCodec<SetBookCoverFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> SetBookCoverFunction.commonFields(instance).and(instance.group(Filterable.codec(Codec.string(0, 32)).optionalFieldOf("title").forGetter(setBookCoverFunction -> setBookCoverFunction.title), Codec.STRING.optionalFieldOf("author").forGetter(setBookCoverFunction -> setBookCoverFunction.author), ExtraCodecs.intRange(0, 3).optionalFieldOf("generation").forGetter(setBookCoverFunction -> setBookCoverFunction.generation))).apply((Applicative<SetBookCoverFunction, ?>)instance, SetBookCoverFunction::new));
