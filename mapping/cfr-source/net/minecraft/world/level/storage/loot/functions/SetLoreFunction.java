...
extends LootItemConditionalFunction {
    public static final MapCodec<SetLoreFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> SetLoreFunction.commonFields(instance).and(instance.group(((MapCodec)ComponentSerialization.CODEC.sizeLimitedListOf(256).fieldOf("lore")).forGetter(setLoreFunction -> setLoreFunction.lore), ListOperation.codec(256).forGetter(setLoreFunction -> setLoreFunction.mode), LootContext.EntityTarget.CODEC.optionalFieldOf("entity").forGetter(setLoreFunction -> setLoreFunction.resolutionContext))).apply((Applicative<SetLoreFunction, ?>)instance, SetLoreFunction::new));
    ...
    public Set<ContextKey<?>> getReferencedContextParams() {
        return this.resolutionContext.map(entityTarget -> Set.of(entityTarget.contextParam())).orElseGet(Set::of);
    }
