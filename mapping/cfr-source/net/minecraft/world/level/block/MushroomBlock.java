...
implements BonemealableBlock {
    public static final MapCodec<MushroomBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)ResourceKey.codec(Registries.CONFIGURED_FEATURE).fieldOf("feature")).forGetter(mushroomBlock -> mushroomBlock.feature), MushroomBlock.propertiesCodec()).apply((Applicative<MushroomBlock, ?>)instance, MushroomBlock::new));
    ...
    public boolean growMushroom(ServerLevel serverLevel, BlockPos blockPos, BlockState blockState, RandomSource randomSource) {
        Optional<Holder.Reference<ConfiguredFeature<?, ?>>> optional = serverLevel.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(this.feature);
        if (optional.isEmpty()) {
            return false;
        }
