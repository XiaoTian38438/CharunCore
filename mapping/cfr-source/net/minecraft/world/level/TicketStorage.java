...
extends SavedData {
    ...
    public static final Codec<TicketStorage> CODEC = RecordCodecBuilder.create(instance -> instance.group(TICKET_ENTRY.listOf().optionalFieldOf("tickets", List.of()).forGetter(TicketStorage::packTickets)).apply((Applicative<TicketStorage, ?>)instance, TicketStorage::fromPacked));
    ...
    private List<Pair<ChunkPos, Ticket>> packTickets() {
        ...
        this.forEachTicket((chunkPos, ticket) -> {
            if (ticket.getType().persist()) {
                arrayList.add(new Pair<ChunkPos, Ticket>((ChunkPos)chunkPos, (Ticket)ticket));
            }
        });
        return arrayList;
    }
    ...
    public void replaceTicketLevelOfType(int n, TicketType ticketType) {
        ArrayList<Pair<Ticket, Long>> arrayList = new ArrayList<Pair<Ticket, Long>>();
        for (Long2ObjectMap.Entry object : this.tickets.long2ObjectEntrySet()) {
            for (Ticket ticket : (List)object.getValue()) {
                if (ticket.getType() != ticketType) continue;
                arrayList.add(Pair.of(ticket, object.getLongKey()));
            }
        }
