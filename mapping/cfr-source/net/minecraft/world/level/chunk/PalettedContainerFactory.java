...
public record PalettedContainerFactory(Strategy<BlockState> blockStatesStrategy, BlockState defaultBlockState, Codec<PalettedContainer<BlockState>> blockStatesContainerCodec, Strategy<Holder<Biome>> biomeStrategy, Holder<Biome> defaultBiome, Codec<PalettedContainerRO<Holder<Biome>>> biomeContainerCodec) {
    public static PalettedContainerFactory create(RegistryAccess registryAccess) {
        ...
        Strategy<Holder<Biome>> strategy2 = Strategy.createForBiomes(registryLookup.asHolderIdMap());
        Holder.Reference<Biome> reference = registryLookup.getOrThrow(Biomes.PLAINS);
        return new PalettedContainerFactory(strategy, blockState, PalettedContainer.codecRW(BlockState.CODEC, strategy, blockState), strategy2, reference, PalettedContainer.codecRO(registryLookup.holderByNameCodec(), strategy2, reference));
    }
