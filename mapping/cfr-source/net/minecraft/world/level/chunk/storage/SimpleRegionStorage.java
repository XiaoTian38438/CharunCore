...
implements AutoCloseable {
    ...
    public CompoundTag upgradeChunkTag(CompoundTag compoundTag, int n, @Nullable CompoundTag compoundTag2) {
        int n2 = NbtUtils.getDataVersion(compoundTag, n);
        if (n2 == SharedConstants.getCurrentVersion().dataVersion().version()) {
            return compoundTag;
        }
        try {
            ...
            compoundTag = this.dataFixType.updateToCurrentVersion(this.fixerUpper, compoundTag, Math.max(this.legacyFixer.get().targetDataVersion(), n2));
            ...
            crashReportCategory.setDetail("Data version", n2);
            throw new ReportedException(crashReport);
        }
    }

    public CompoundTag upgradeChunkTag(CompoundTag compoundTag, int n) {
        return this.upgradeChunkTag(compoundTag, n, null);
    }

    public Dynamic<Tag> upgradeChunkTag(Dynamic<Tag> dynamic, int n) {
        return new Dynamic<Tag>(dynamic.getOps(), this.upgradeChunkTag((CompoundTag)dynamic.getValue(), n, null));
    }
