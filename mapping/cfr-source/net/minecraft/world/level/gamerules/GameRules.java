...
public class GameRules {
    ...
    public <T> void set(GameRule<T> gameRule, T t, @Nullable MinecraftServer minecraftServer) {
        if (!this.rules.has(gameRule)) {
            throw new IllegalArgumentException("Tried to set invalid game rule");
        }
        this.rules.set(gameRule, t);
        if (minecraftServer != null) {
            minecraftServer.onGameRuleChanged(gameRule, t);
        }
    }
    ...
    private static GameRule<Boolean> registerBoolean(String string, GameRuleCategory gameRuleCategory, boolean bl2) {
        return GameRules.register(string, gameRuleCategory, GameRuleType.BOOL, BoolArgumentType.bool(), Codec.BOOL, bl2, FeatureFlagSet.of(), GameRuleTypeVisitor::visitBoolean, bl -> bl != false ? 1 : 0);
    }

    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int n, int n2) {
        return GameRules.registerInteger(string, gameRuleCategory, n, n2, Integer.MAX_VALUE, FeatureFlagSet.of());
    }

    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int n, int n2, int n3) {
        return GameRules.registerInteger(string, gameRuleCategory, n, n2, n3, FeatureFlagSet.of());
    }

    private static GameRule<Integer> registerInteger(String string, GameRuleCategory gameRuleCategory, int n2, int n3, int n4, FeatureFlagSet featureFlagSet) {
        return GameRules.register(string, gameRuleCategory, GameRuleType.INT, IntegerArgumentType.integer(n3, n4), Codec.intRange(n3, n4), n2, featureFlagSet, GameRuleTypeVisitor::visitInteger, n -> n);
    }

    private static <T> GameRule<T> register(String string, GameRuleCategory gameRuleCategory, GameRuleType gameRuleType, ArgumentType<T> argumentType, Codec<T> codec, T t, FeatureFlagSet featureFlagSet, VisitorCaller<T> visitorCaller, ToIntFunction<T> toIntFunction) {
        return Registry.register(BuiltInRegistries.GAME_RULE, string, new GameRule<T>(gameRuleCategory, gameRuleType, argumentType, visitorCaller, codec, toIntFunction, t, featureFlagSet));
    }
