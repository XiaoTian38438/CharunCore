...
public record EitherHolder<T>(Either<Holder<T>, ResourceKey<T>> contents) {
    ...
    public static <T> Codec<EitherHolder<T>> codec(ResourceKey<Registry<T>> resourceKey2, Codec<Holder<T>> codec) {
        return Codec.either(codec, ResourceKey.codec(resourceKey2).comapFlatMap(resourceKey -> DataResult.error(() -> "Cannot parse as key without registry"), Function.identity())).xmap(EitherHolder::new, EitherHolder::contents);
    }
