...
public record ConditionalEffect<T>(T effect, Optional<LootItemCondition> requirements) {
    ...
    public static <T> Codec<ConditionalEffect<T>> codec(Codec<T> codec, ContextKeySet contextKeySet) {
        return RecordCodecBuilder.create(instance -> instance.group(((MapCodec)codec.fieldOf("effect")).forGetter(ConditionalEffect::effect), ConditionalEffect.conditionCodec(contextKeySet).optionalFieldOf("requirements").forGetter(ConditionalEffect::requirements)).apply((Applicative<ConditionalEffect, ?>)instance, ConditionalEffect::new));
    }
