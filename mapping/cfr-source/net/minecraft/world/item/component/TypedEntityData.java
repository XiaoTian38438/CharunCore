...
implements TooltipProvider {
    ...
    public static <T> Codec<TypedEntityData<T>> codec(final Codec<T> codec) {
        return new Codec<TypedEntityData<T>>(){
            ...
            @Override
            public <V> DataResult<Pair<TypedEntityData<T>, V>> decode(DynamicOps<V> dynamicOps, V v) {
                return CustomData.COMPOUND_TAG_CODEC.decode(dynamicOps, v).flatMap((? super R pair) -> {
                    ...
                    return codec.parse(1.asNbtOps(dynamicOps), tag).map((? super R object) -> Pair.of(new TypedEntityData<Object>(object, compoundTag), pair.getSecond()));
                });
            }

            @Override
            public <V> DataResult<V> encode(TypedEntityData<T> typedEntityData, DynamicOps<V> dynamicOps, V v) {
                return codec.encodeStart(1.asNbtOps(dynamicOps), typedEntityData.type).flatMap((? super R tag) -> {
                    CompoundTag compoundTag = typedEntityData.tag.copy();
                    compoundTag.put(TypedEntityData.TYPE_TAG, (Tag)tag);
                    return CustomData.COMPOUND_TAG_CODEC.encode(compoundTag, dynamicOps, v);
                });
            }
    ...
    TypedEntityData(IdType IdType, CompoundTag compoundTag) {
        this.type = IdType;
        this.tag = TypedEntityData.stripId(compoundTag);
    }

    public static <T> TypedEntityData<T> of(T t, CompoundTag compoundTag) {
        return new TypedEntityData<T>(t, compoundTag);
    }
