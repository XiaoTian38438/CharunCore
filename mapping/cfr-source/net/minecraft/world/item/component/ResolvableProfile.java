...
implements TooltipProvider {
    private static final Codec<ResolvableProfile> FULL_CODEC = RecordCodecBuilder.create(instance -> instance.group(Codec.mapEither(ExtraCodecs.STORED_GAME_PROFILE, Partial.MAP_CODEC).forGetter(ResolvableProfile::unpack), PlayerSkin.Patch.MAP_CODEC.forGetter(ResolvableProfile::skinPatch)).apply((Applicative<ResolvableProfile, ?>)instance, ResolvableProfile::create));
    ...
    private static ResolvableProfile create(Either<GameProfile, Partial> either, PlayerSkin.Patch patch) {
        return either.map(gameProfile -> new Static(Either.left(gameProfile), patch), partial -> {
            if (!partial.properties.isEmpty() || partial.id.isPresent() == partial.name.isPresent()) {
                return new Static(Either.right(partial), patch);
            }
            return partial.name.map(string -> new Dynamic(Either.left(string), patch)).orElseGet(() -> new Dynamic(Either.right(partial.id.get()), patch));
        });
    }
    ...
    public static ResolvableProfile createUnresolved(String string) {
        return new Dynamic(Either.left(string), PlayerSkin.Patch.EMPTY);
    }

    public static ResolvableProfile createUnresolved(UUID uUID) {
        return new Dynamic(Either.right(uUID), PlayerSkin.Patch.EMPTY);
    }
    ...
    extends ResolvableProfile {
        ...
        Static(Either<GameProfile, Partial> either, PlayerSkin.Patch patch) {
            super(either.map(gameProfile -> gameProfile, Partial::createProfile), patch);
        ...
        public boolean equals(Object object) {
            ...
            if (!this.skinPatch.equals(static_.skinPatch)) return false;
            return true;
        }

        public int hashCode() {
            int n = 31 + this.contents.hashCode();
            n = 31 * n + this.skinPatch.hashCode();
            return n;
        }
    ...
    extends ResolvableProfile {
        ...
        public int hashCode() {
            int n = 31 + this.nameOrId.hashCode();
            n = 31 * n + this.skinPatch.hashCode();
            return n;
        }
    ...
    extends Record {
        ...
        static final MapCodec<Partial> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(ExtraCodecs.PLAYER_NAME.optionalFieldOf("name").forGetter(Partial::name), UUIDUtil.CODEC.optionalFieldOf("id").forGetter(Partial::id), ExtraCodecs.PROPERTY_MAP.optionalFieldOf("properties", PropertyMap.EMPTY).forGetter(Partial::properties)).apply((Applicative<Partial, ?>)instance, Partial::new));
