...
public class Brain<E extends LivingEntity> {
    ...
    public static <E extends LivingEntity> Codec<Brain<E>> codec(final Collection<? extends MemoryModuleType<?>> collection, final Collection<? extends SensorType<? extends Sensor<? super E>>> collection2) {
        ...
        mutableObject.setValue(new MapCodec<Brain<E>>(){
            ...
            public <T> Stream<T> keys(DynamicOps<T> dynamicOps) {
                return collection.stream().flatMap((? super T memoryModuleType) -> memoryModuleType.getCodec().map((? super T codec) -> BuiltInRegistries.MEMORY_MODULE_TYPE.getKey((MemoryModuleType<?>)memoryModuleType)).stream()).map((? super T identifier) -> dynamicOps.createString(identifier.toString()));
            }
            ...
            public <T> DataResult<Brain<E>> decode(DynamicOps<T> dynamicOps, MapLike<T> mapLike) {
                ...
                mapLike.entries().forEach(pair -> {
                    ...
                    mutableObject2.setValue(((DataResult)mutableObject2.get()).apply2(ImmutableList.Builder::add, dataResult2));
                });
            ...
            private <T, U> DataResult<MemoryValue<U>> captureRead(MemoryModuleType<U> memoryModuleType, DynamicOps<T> dynamicOps, T t) {
                return memoryModuleType.getCodec().map(DataResult::success).orElseGet(() -> DataResult.error(() -> "No codec for memory: " + String.valueOf(memoryModuleType))).flatMap((? super R codec) -> codec.parse(dynamicOps, t)).map((? super R expirableValue) -> new MemoryValue(memoryModuleType, Optional.of(expirableValue)));
            }
    ...
    public void addActivityAndRemoveMemoryWhenStopped(Activity activity, int n, ImmutableList<? extends BehaviorControl<? super E>> immutableList, MemoryModuleType<?> memoryModuleType) {
        ImmutableSet immutableSet = ImmutableSet.of(Pair.of(memoryModuleType, MemoryStatus.VALUE_PRESENT));
    ...
    ImmutableList<? extends Pair<Integer, ? extends BehaviorControl<? super E>>> createPriorityPairs(int n, ImmutableList<? extends BehaviorControl<? super E>> immutableList) {
        ...
        for (BehaviorControl behaviorControl : immutableList) {
            builder.add(Pair.of(n2++, behaviorControl));
        }
