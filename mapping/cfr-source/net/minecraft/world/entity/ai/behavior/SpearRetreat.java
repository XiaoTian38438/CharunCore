...
extends Behavior<PathfinderMob> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel serverLevel, PathfinderMob pathfinderMob) {
        ...
        double d2 = pathfinderMob.distanceToSqr(livingEntity.getX(), livingEntity.getY(), livingEntity.getZ());
        int n = pathfinderMob.isPassenger() ? 2 : 0;
        Vec3 vec3 = LandRandomPos.getPosAway(pathfinderMob, Math.max(0.0, (double)(9 + n) - (d = Math.sqrt(d2))), Math.max(1.0, (double)(11 + n) - d), 7, livingEntity.position());
        if (vec3 == null) {
            return false;
        }
