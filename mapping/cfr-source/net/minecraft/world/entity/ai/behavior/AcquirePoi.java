...
public class AcquirePoi {
    ...
    public static @Nullable Path findPathToPois(Mob mob, Set<Pair<Holder<PoiType>, BlockPos>> set) {
        ...
        for (Pair<Holder<PoiType>, BlockPos> pair : set) {
            n = Math.max(n, pair.getFirst().value().validRange());
    ...
    private static /* synthetic */ App lambda$create$10(MemoryModuleType memoryModuleType, boolean bl, MutableLong mutableLong, Long2ObjectMap long2ObjectMap, Predicate predicate, BiPredicate biPredicate, Optional optional, BehaviorBuilder.Instance instance) {
        return instance.group(instance.absent(memoryModuleType)).apply(instance, memoryAccessor -> (serverLevel, pathfinderMob, l) -> {
            ...
            if (path != null && path.canReach()) {
                ...
                for (Pair<Holder<PoiType>, BlockPos> pair2 : set) {
                    long2ObjectMap.computeIfAbsent(pair2.getSecond().asLong(), l2 -> new JitteredLinearRetry(serverLevel.random, l));
                }
            }
            return true;
        });
