...
public final class ProjectileUtil {
    ...
    public static Collection<EntityHitResult> getManyEntityHitResult(Level level, Entity entity, Vec3 vec3, Vec3 vec32, AABB aABB, Predicate<Entity> predicate, float f, ClipContext.Block block, boolean bl) {
        ...
        for (Entity entity2 : level.getEntities(entity, aABB, predicate)) {
            ...
            Optional<Vec3> optional3 = aABB2.clip(vec3, vec32);
            if (optional3.isPresent()) {
                arrayList.add(new EntityHitResult(entity2, optional3.get()));
                continue;
            }
            if ((double)f <= 0.0 || (optional2 = aABB2.inflate(f).clip(vec3, vec32)).isEmpty()) continue;
            Vec3 vec34 = optional2.get();
            BlockHitResult blockHitResult = level.clipIncludingBorder(new ClipContext(vec34, vec33 = aABB2.getCenter(), block, ClipContext.Fluid.NONE, entity));
            if (blockHitResult.getType() != HitResult.Type.MISS) {
                vec33 = blockHitResult.getLocation();
            }
            if (!(optional = entity2.getBoundingBox().clip(vec34, vec33)).isPresent()) continue;
            arrayList.add(new EntityHitResult(entity2, optional.get()));
        }
        return arrayList;
    }
