...
public final class EnvironmentAttributeMap {
    ...
    public static final Codec<EnvironmentAttributeMap> CODEC_ONLY_POSITIONAL = CODEC.validate(environmentAttributeMap -> {
        ...
        return DataResult.success(environmentAttributeMap);
    });
    ...
    public <Value> Value applyModifier(EnvironmentAttribute<Value> environmentAttribute, Value Value2) {
        Entry<Value, ?> entry = this.get(environmentAttribute);
        return entry != null ? entry.applyModifier(Value2) : Value2;
    }
    ...
    public static class Builder {
        ...
        public <Value, Parameter> Builder modify(EnvironmentAttribute<Value> environmentAttribute, AttributeModifier<Value, Parameter> attributeModifier, Parameter Parameter2) {
            environmentAttribute.type().checkAllowedModifier(attributeModifier);
            this.entries.put(environmentAttribute, new Entry<Value, Parameter>(Parameter2, attributeModifier));
            return this;
        }

        public <Value> Builder set(EnvironmentAttribute<Value> environmentAttribute, Value Value2) {
            return this.modify(environmentAttribute, AttributeModifier.override(), Value2);
        }
    ...
    public record Entry<Value, Argument>(Argument argument, AttributeModifier<Value, Argument> modifier) {
        private static <Value> Codec<Entry<Value, ?>> createCodec(EnvironmentAttribute<Value> environmentAttribute) {
            Codec<Entry> codec = environmentAttribute.type().modifierCodec().dispatch("modifier", Entry::modifier, Util.memoize(attributeModifier -> Entry.createFullCodec(environmentAttribute, attributeModifier)));
            return Codec.either(environmentAttribute.valueCodec(), codec).xmap(either -> either.map(object -> new Entry(object, AttributeModifier.override()), entry -> entry), entry -> {
        ...
        private static <Value, Argument> MapCodec<Entry<Value, Argument>> createFullCodec(EnvironmentAttribute<Value> environmentAttribute, AttributeModifier<Value, Argument> attributeModifier) {
            return RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)attributeModifier.argumentCodec(environmentAttribute).fieldOf("argument")).forGetter(Entry::argument)).apply((Applicative<Entry, ?>)instance, object -> new Entry(object, attributeModifier)));
        }

        public Value applyModifier(Value Value2) {
            return this.modifier.apply(Value2, this.argument);
        }
    }
}
