...
public record AmbientSounds(Optional<Holder<SoundEvent>> loop, Optional<AmbientMoodSettings> mood, List<AmbientAdditionsSettings> additions) {
    ...
    public static final Codec<AmbientSounds> CODEC = RecordCodecBuilder.create(instance -> instance.group(SoundEvent.CODEC.optionalFieldOf("loop").forGetter(AmbientSounds::loop), AmbientMoodSettings.CODEC.optionalFieldOf("mood").forGetter(AmbientSounds::mood), ExtraCodecs.compactListCodec(AmbientAdditionsSettings.CODEC).optionalFieldOf("additions", List.of()).forGetter(AmbientSounds::additions)).apply((Applicative<AmbientSounds, ?>)instance, AmbientSounds::new));
}
