...
public abstract class DiscreteVoxelShape {
    ...
    public DiscreteVoxelShape rotate(OctahedralGroup octahedralGroup) {
        ...
        int n = DiscreteVoxelShape.fixupCoordinate(vector3i, 0);
        int n2 = DiscreteVoxelShape.fixupCoordinate(vector3i, 1);
        int n3 = DiscreteVoxelShape.fixupCoordinate(vector3i, 2);
        BitSetDiscreteVoxelShape bitSetDiscreteVoxelShape = new BitSetDiscreteVoxelShape(vector3i.x, vector3i.y, vector3i.z);
        for (int i = 0; i < this.xSize; ++i) {
            for (int j = 0; j < this.ySize; ++j) {
                for (int k = 0; k < this.zSize; ++k) {
                    if (!this.isFull(i, j, k)) continue;
                    Vector3i vector3i2 = octahedralGroup.rotate(vector3i.set(i, j, k));
                    int n4 = n + vector3i2.x;
                    int n5 = n2 + vector3i2.y;
                    int n6 = n3 + vector3i2.z;
                    ((DiscreteVoxelShape)bitSetDiscreteVoxelShape).fill(n4, n5, n6);
                }
            }
        }
    ...
    private static int fixupCoordinate(Vector3i vector3i, int n) {
