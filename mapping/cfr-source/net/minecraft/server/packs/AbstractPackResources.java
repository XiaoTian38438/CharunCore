...
implements PackResources {
    ...
    public static <T> @Nullable T getMetadataFromStream(MetadataSectionType<T> metadataSectionType, InputStream inputStream, PackLocationInfo packLocationInfo) {
        ...
        return metadataSectionType.codec().parse(JsonOps.INSTANCE, jsonObject.get(metadataSectionType.name())).ifError(error -> LOGGER.error("Couldn't load {} {} metadata: {}", new Object[]{packLocationInfo.id(), metadataSectionType.name(), error.message()})).result().orElse(null);
    }
