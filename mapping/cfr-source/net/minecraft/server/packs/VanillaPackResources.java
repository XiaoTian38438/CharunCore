...
implements PackResources {
    ...
    @Override
    public @Nullable IoSupplier<InputStream> getRootResource(String ... stringArray) {
    ...
    public void listResources(PackType packType, String string, String string2, PackResources.ResourceOutput resourceOutput) {
        FileUtil.decomposePath(string2).ifSuccess(list -> {
            ...
            if (n == 1) {
                ...
                HashMap<Identifier, IoSupplier<InputStream>> hashMap = new HashMap<Identifier, IoSupplier<InputStream>>();
                for (int i = 0; i < n - 1; ++i) {
    ...
    public <T> @Nullable T getMetadataSection(MetadataSectionType<T> metadataSectionType) {
        ...
        try (InputStream inputStream = ioSupplier.get();){
            T t2 = AbstractPackResources.getMetadataFromStream(metadataSectionType, inputStream, this.location);
