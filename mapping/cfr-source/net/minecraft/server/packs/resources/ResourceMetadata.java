...
public interface ResourceMetadata {
            ...
            ResourceMetadata resourceMetadata = new ResourceMetadata(){
                ...
                public <T> Optional<T> getSection(MetadataSectionType<T> metadataSectionType) {
                    ...
                    if (jsonObject.has(string)) {
                        Object a = metadataSectionType.codec().parse(JsonOps.INSTANCE, jsonObject.get(string)).getOrThrow(JsonParseException::new);
    ...
    default public List<MetadataSectionType.WithValue<?>> getTypedSections(Collection<MetadataSectionType<?>> collection) {
        return collection.stream().map(this::getTypedSection).flatMap(Optional::stream).collect(Collectors.toUnmodifiableList());
    }
}
