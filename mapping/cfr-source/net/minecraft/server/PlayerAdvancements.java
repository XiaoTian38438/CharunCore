...
public class PlayerAdvancements {
    ...
    private void load(ServerAdvancementManager serverAdvancementManager) {
        if (Files.isRegularFile(this.playerSavePath, new LinkOption[0])) {
            try (BufferedReader bufferedReader = Files.newBufferedReader(this.playerSavePath, StandardCharsets.UTF_8);){
                ...
                Data data = (Data)this.codec.parse(JsonOps.INSTANCE, jsonElement).getOrThrow(JsonParseException::new);
    ...
    private Data asData() {
        LinkedHashMap<Identifier, AdvancementProgress> linkedHashMap = new LinkedHashMap<Identifier, AdvancementProgress>();
    ...
    public void flushDirty(ServerPlayer serverPlayer, boolean bl) {
        if (this.isFirstPacket || !this.rootsToUpdate.isEmpty() || !this.progressChanged.isEmpty()) {
            HashMap<Identifier, AdvancementProgress> hashMap = new HashMap<Identifier, AdvancementProgress>();
