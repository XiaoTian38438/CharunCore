...
public record Filterable<T>(T raw, Optional<T> filtered) {
    public static <T> Codec<Filterable<T>> codec(Codec<T> codec) {
        Codec codec2 = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)codec.fieldOf("raw")).forGetter(Filterable::raw), codec.optionalFieldOf("filtered").forGetter(Filterable::filtered)).apply((Applicative<Filterable, ?>)instance, Filterable::new));
        Codec<Filterable> codec3 = codec.xmap(Filterable::passThrough, Filterable::raw);
