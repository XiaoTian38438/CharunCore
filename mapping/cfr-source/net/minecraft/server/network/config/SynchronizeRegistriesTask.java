...
implements ConfigurationTask {
    ...
    private void sendRegistries(Consumer<Packet<?>> consumer, Set<KnownPack> set) {
        RegistryOps<Tag> registryOps = this.registries.compositeAccess().createSerializationContext(NbtOps.INSTANCE);
        RegistrySynchronization.packRegistries(registryOps, this.registries.getAccessFrom(RegistryLayer.WORLDGEN), set, (resourceKey, list) -> consumer.accept(new ClientboundRegistryDataPacket((ResourceKey<? extends Registry<?>>)resourceKey, (List<RegistrySynchronization.PackedRegistryEntry>)list)));
