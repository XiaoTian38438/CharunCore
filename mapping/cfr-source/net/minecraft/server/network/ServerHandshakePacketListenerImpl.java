...
implements ServerHandshakePacketListener {
    ...
    private void beginLogin(ClientIntentionPacket clientIntentionPacket, boolean bl) {
        ...
        if (clientIntentionPacket.protocolVersion() != SharedConstants.getCurrentVersion().protocolVersion()) {
            MutableComponent mutableComponent = clientIntentionPacket.protocolVersion() < 754 ? Component.translatable("multiplayer.disconnect.outdated_client", SharedConstants.getCurrentVersion().name()) : Component.translatable("multiplayer.disconnect.incompatible", SharedConstants.getCurrentVersion().name());
            this.connection.send(new ClientboundLoginDisconnectPacket(mutableComponent));
            this.connection.disconnect(mutableComponent);
        } else {
