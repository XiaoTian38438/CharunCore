...
public class AllowlistService {
    public static List<PlayerDto> get(MinecraftApi minecraftApi) {
        return minecraftApi.allowListService().getEntries().stream().filter(userWhiteListEntry -> userWhiteListEntry.getUser() != null).map(userWhiteListEntry -> PlayerDto.from((NameAndId)userWhiteListEntry.getUser())).toList();
    }
    ...
    public static List<PlayerDto> add(MinecraftApi minecraftApi, List<PlayerDto> list, ClientInfo clientInfo) {
        ...
        for (Optional optional : Util.sequence(list2).join()) {
            optional.ifPresent(nameAndId -> minecraftApi.allowListService().add(new UserWhiteListEntry((NameAndId)nameAndId), clientInfo));
        }
    ...
    public static List<PlayerDto> remove(MinecraftApi minecraftApi, List<PlayerDto> list, ClientInfo clientInfo) {
        List<CompletableFuture> list2 = list.stream().map(playerDto -> minecraftApi.playerListService().getUser(playerDto.id(), playerDto.name())).toList();
        for (Optional optional : Util.sequence(list2).join()) {
            optional.ifPresent(nameAndId -> minecraftApi.allowListService().remove((NameAndId)nameAndId, clientInfo));
        }
    ...
    public static List<PlayerDto> set(MinecraftApi minecraftApi, List<PlayerDto> list, ClientInfo clientInfo) {
        ...
        set2.stream().filter(nameAndId -> !set.contains(nameAndId)).forEach(nameAndId -> minecraftApi.allowListService().remove((NameAndId)nameAndId, clientInfo));
        set.stream().filter(nameAndId -> !set2.contains(nameAndId)).forEach(nameAndId -> minecraftApi.allowListService().add(new UserWhiteListEntry((NameAndId)nameAndId), clientInfo));
        minecraftApi.allowListService().kickUnlistedPlayers(clientInfo);
        return AllowlistService.get(minecraftApi);
    }
}
