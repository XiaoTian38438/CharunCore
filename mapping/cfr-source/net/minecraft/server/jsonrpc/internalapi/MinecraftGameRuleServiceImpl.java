...
implements MinecraftGameRuleService {
    ...
    public <T> GameRulesService.GameRuleUpdate<T> updateGameRule(GameRulesService.GameRuleUpdate<T> gameRuleUpdate, ClientInfo clientInfo) {
        ...
        T t2 = gameRuleUpdate.value();
        this.gameRules.set(gameRule, t2, this.server);
        this.jsonrpcLogger.log(clientInfo, "Game rule '{}' updated from '{}' to '{}'", gameRule.id(), gameRule.serialize(t), gameRule.serialize(t2));
        return gameRuleUpdate;
    }

    @Override
    public <T> GameRulesService.GameRuleUpdate<T> getTypedRule(GameRule<T> gameRule, T t) {
        return new GameRulesService.GameRuleUpdate<T>(gameRule, t);
    }
