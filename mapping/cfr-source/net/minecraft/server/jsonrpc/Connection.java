...
extends SimpleChannelInboundHandler<JsonElement> {
    ...
    public <Params> void sendNotification(Holder.Reference<? extends OutgoingRpcMethod<Params, ?>> reference, Params Params) {
        this.sendRequest(reference, Params, false);
    }
    ...
    public <Params, Result> CompletableFuture<Result> sendRequest(Holder.Reference<? extends OutgoingRpcMethod<Params, Result>> reference, Params Params) {
        return this.sendRequest(reference, Params, true);
    }

    @Contract(value="_,_,false->null;_,_,true->!null")
    private <Params, Result> @Nullable CompletableFuture<Result> sendRequest(Holder.Reference<? extends OutgoingRpcMethod<Params, ? extends Result>> reference, @Nullable Params Params, boolean bl) {
        List<JsonElement> list;
        List<JsonElement> list2 = list = Params != null ? List.of(Objects.requireNonNull(reference.value().encodeParams(Params))) : List.of();
        if (bl) {
            CompletableFuture completableFuture = new CompletableFuture();
            int n = this.transactionId.incrementAndGet();
            long l = Util.timeSource.get(TimeUnit.MILLISECONDS);
            this.pendingRequests.put(n, new PendingRpcRequest(reference, completableFuture, l + 5000L));
            this.channel.writeAndFlush((Object)JsonRPCUtils.createRequest(n, reference.key().identifier(), list));
    ...
    @Nullable JsonObject handleJsonObject(JsonObject jsonObject) {
        try {
            ...
            return JsonRPCErrors.INVALID_REQUEST.createWithoutData((JsonElement)Objects.requireNonNullElse(jsonElement, JsonNull.INSTANCE));
        }
    ...
    private void handleRequestResponse(int n, JsonElement jsonElement) {
        PendingRpcRequest pendingRpcRequest = (PendingRpcRequest)this.pendingRequests.remove(n);
        if (pendingRpcRequest == null) {
            LOGGER.warn("Received unknown response (id: {}): {}", (Object)n, (Object)jsonElement);
        } else {
    ...
    private @Nullable JsonObject handleError(@Nullable JsonElement jsonElement, JsonObject jsonObject) {
        ...
        if (jsonElement != null && Connection.isValidResponseId(jsonElement) && (pendingRpcRequest = (PendingRpcRequest)this.pendingRequests.remove(jsonElement.getAsInt())) != null) {
