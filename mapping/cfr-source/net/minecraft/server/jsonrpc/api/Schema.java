...
public record Schema<T>(Optional<URI> reference, List<String> type, Optional<Schema<?>> items, Map<String, Schema<?>> properties, List<String> enumValues, Codec<T> codec) {
    public static final Codec<? extends Schema<?>> CODEC = Codec.recursive("Schema", codec -> RecordCodecBuilder.create(instance -> instance.group(ReferenceUtil.REFERENCE_CODEC.optionalFieldOf("$ref").forGetter(Schema::reference), ExtraCodecs.compactListCodec(Codec.STRING).optionalFieldOf("type", List.of()).forGetter(Schema::type), codec.optionalFieldOf("items").forGetter(Schema::items), Codec.unboundedMap(Codec.STRING, codec).optionalFieldOf("properties", Map.of()).forGetter(Schema::properties), Codec.STRING.listOf().optionalFieldOf("enum", List.of()).forGetter(Schema::enumValues)).apply((Applicative<Schema, ?>)instance, (optional, list, optional2, map, list2) -> null))).validate(schema -> {
        if (schema == null) {
    ...
    public static final Schema<Either<Boolean, Integer>> BOOL_OR_INT_SCHEMA = Schema.ofTypes(List.of("boolean", "integer"), Codec.either(Codec.BOOL, Codec.INT));
    ...
    public Schema<T> info() {
        return new Schema<T>(this.reference, this.type, this.items.map(Schema::info), this.properties.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> ((Schema)entry.getValue()).info())), this.enumValues, this.codec);
    }
    ...
    public static <T> Schema<T> ofType(String string, Codec<T> codec) {
        return Schema.ofTypes(List.of(string), codec);
    }
    ...
    public static <T> Schema<T> ofEnum(List<String> list, Codec<T> codec) {
        return new Schema<T>(Optional.empty(), List.of("string"), Optional.empty(), Map.of(), list, codec);
    }

    public static <T> Schema<List<T>> arrayOf(Schema<?> schema, Codec<T> codec) {
        return new Schema<List<T>>(Optional.empty(), List.of("array"), Optional.of(schema), Map.of(), List.of(), codec.listOf());
    }
    ...
    private static <T> Schema<T> record(Map<String, Schema<?>> map, Codec<T> codec) {
        return new Schema<T>(Optional.empty(), List.of("object"), Optional.empty(), map, List.of(), codec);
    }
