...
implements LevelLoadListener {
    ...
    @Override
    public void start(LevelLoadListener.Stage stage, int n) {
        if (this.closed) {
            return;
        }
        ...
        this.progressTracker.start(stage, n);
        switch (stage) {
            ...
            case LOAD_INITIAL_CHUNKS: {
                LOGGER.info("Loading {} persistent chunks...", (Object)n);
                break;
            }
            case LOAD_PLAYER_CHUNKS: {
                LOGGER.info("Loading {} chunks for player spawn...", (Object)n);
            }
        }
    }

    @Override
    public void update(LevelLoadListener.Stage stage, int n, int n2) {
        if (this.closed) {
            return;
        }
        this.progressTracker.update(stage, n, n2);
        if (Util.getMillis() > this.nextLogTime) {
            this.nextLogTime += 500L;
            int n3 = Mth.floor(this.progressTracker.get() * 100.0f);
            LOGGER.info(Component.translatable("menu.preparingSpawn", n3).getString());
        }
    }
