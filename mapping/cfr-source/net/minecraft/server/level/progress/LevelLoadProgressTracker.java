...
implements LevelLoadListener {
    ...
    public void start(LevelLoadListener.Stage stage, int n) {
        ...
        switch (stage) {
            case LOAD_INITIAL_CHUNKS: {
                int n2 = this.includePlayerChunks ? EXPECTED_PLAYER_CHUNKS : 0;
                this.totalWeight = 10 + n + n2;
    ...
    private void beginSegment(int n) {
        this.segmentWeight = n;
    ...
    public void update(LevelLoadListener.Stage stage, int n, int n2) {
        if (this.tracksStage(stage)) {
            this.segmentFraction = n2 == 0 ? 0.0f : (float)n / (float)n2;
            this.updateProgress();
        }
    }
