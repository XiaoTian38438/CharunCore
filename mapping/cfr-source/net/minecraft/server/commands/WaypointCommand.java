...
public class WaypointCommand {
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher, CommandBuildContext commandBuildContext) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("waypoint").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.literal("list").executes(commandContext -> WaypointCommand.listWaypoints((CommandSourceStack)commandContext.getSource())))).then(Commands.literal("modify").then((ArgumentBuilder<CommandSourceStack, ?>)((RequiredArgumentBuilder)Commands.argument("waypoint", EntityArgument.entity()).then((ArgumentBuilder<CommandSourceStack, ?>)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("color").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("color", ColorArgument.color()).executes(commandContext -> WaypointCommand.setWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint(commandContext, "waypoint"), ColorArgument.getColor(commandContext, "color"))))).then(Commands.literal("hex").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("color", HexColorArgument.hexColor()).executes(commandContext -> WaypointCommand.setWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint(commandContext, "waypoint"), HexColorArgument.getHexColor(commandContext, "color")))))).then(Commands.literal("reset").executes(commandContext -> WaypointCommand.resetWaypointColor((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint(commandContext, "waypoint")))))).then(((LiteralArgumentBuilder)Commands.literal("style").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.literal("reset").executes(commandContext -> WaypointCommand.setWaypointStyle((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint(commandContext, "waypoint"), WaypointStyleAssets.DEFAULT)))).then(Commands.literal("set").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("style", IdentifierArgument.id()).executes(commandContext -> WaypointCommand.setWaypointStyle((CommandSourceStack)commandContext.getSource(), WaypointArgument.getWaypoint(commandContext, "waypoint"), ResourceKey.create(WaypointStyleAssets.ROOT_ID, IdentifierArgument.getId(commandContext, "style"))))))))));
    }
    ...
    private static int setWaypointColor(CommandSourceStack commandSourceStack, WaypointTransmitter waypointTransmitter, Integer n) {
        ...
        commandSourceStack.sendSuccess(() -> Component.translatable("commands.waypoint.modify.color", Component.literal(HexFormat.of().withUpperCase().toHexDigits(ARGB.color(0, (int)n), 6)).withColor(n)), false);
        return 0;
    }
