...
public class GameRuleCommand {
    ...
    static <T> LiteralArgumentBuilder<CommandSourceStack> buildRuleArguments(GameRule<T> gameRule, LiteralArgumentBuilder<CommandSourceStack> literalArgumentBuilder) {
        return (LiteralArgumentBuilder)((LiteralArgumentBuilder)literalArgumentBuilder.executes(commandContext -> GameRuleCommand.queryRule((CommandSourceStack)commandContext.getSource(), gameRule))).then(Commands.argument("value", gameRule.argument()).executes(commandContext -> GameRuleCommand.setRule(commandContext, gameRule)));
    }

    private static <T> int setRule(CommandContext<CommandSourceStack> commandContext, GameRule<T> gameRule) {
        CommandSourceStack commandSourceStack = commandContext.getSource();
        Object t = commandContext.getArgument("value", gameRule.valueClass());
        commandSourceStack.getLevel().getGameRules().set(gameRule, t, commandContext.getSource().getServer());
    ...
    private static <T> int queryRule(CommandSourceStack commandSourceStack, GameRule<T> gameRule) {
        Object t = commandSourceStack.getLevel().getGameRules().get(gameRule);
        commandSourceStack.sendSuccess(() -> Component.translatable("commands.gamerule.query", gameRule.id(), gameRule.serialize(t)), false);
        return gameRule.getCommandResult(t);
    }
}
