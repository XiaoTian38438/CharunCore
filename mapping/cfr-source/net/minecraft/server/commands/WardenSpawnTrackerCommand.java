...
public class WardenSpawnTrackerCommand {
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("warden_spawn_tracker").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.literal("clear").executes(commandContext -> WardenSpawnTrackerCommand.resetTracker((CommandSourceStack)commandContext.getSource(), (Collection<? extends Player>)ImmutableList.of((Object)((CommandSourceStack)commandContext.getSource()).getPlayerOrException()))))).then(Commands.literal("set").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("warning_level", IntegerArgumentType.integer(0, 4)).executes(commandContext -> WardenSpawnTrackerCommand.setWarningLevel((CommandSourceStack)commandContext.getSource(), (Collection<? extends Player>)ImmutableList.of((Object)((CommandSourceStack)commandContext.getSource()).getPlayerOrException()), IntegerArgumentType.getInteger(commandContext, "warning_level"))))));
    }
