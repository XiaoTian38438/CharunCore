...
implements Comparable<AdvancementProgress> {
    ...
    public static final Codec<AdvancementProgress> CODEC = RecordCodecBuilder.create(instance -> instance.group(CRITERIA_CODEC.optionalFieldOf("criteria", Map.of()).forGetter(advancementProgress -> advancementProgress.criteria), ((MapCodec)Codec.BOOL.fieldOf("done")).orElse(true).forGetter(AdvancementProgress::isDone)).apply((Applicative<AdvancementProgress, ?>)instance, (map, bl) -> new AdvancementProgress(new HashMap<String, CriterionProgress>((Map<String, CriterionProgress>)map))));
