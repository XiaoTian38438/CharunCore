...
public record HashedPatchMap(Map<DataComponentType<?>, Integer> addedComponents, Set<DataComponentType<?>> removedComponents) {
    ...
    public static HashedPatchMap create(DataComponentPatch dataComponentPatch, HashGenerator hashGenerator) {
        ...
        IdentityHashMap identityHashMap = new IdentityHashMap(splitResult.added().size());
        splitResult.added().forEach(typedDataComponent -> identityHashMap.put(typedDataComponent.type(), (Integer)hashGenerator.apply(typedDataComponent)));
        return new HashedPatchMap(identityHashMap, splitResult.removed());
    }
    ...
    public boolean matches(DataComponentPatch dataComponentPatch, HashGenerator hashGenerator) {
        ...
        for (TypedDataComponent<?> typedDataComponent : splitResult.added()) {
            Integer n = this.addedComponents.get(typedDataComponent.type());
            if (n == null) {
                return false;
            }
            Integer n2 = (Integer)hashGenerator.apply(typedDataComponent);
