    ...
    public static GameTestRunner.GameTestBatcher fromGameTestInfo(int n) {
        return collection -> {
            ...
            return map.entrySet().stream().flatMap(entry -> {
                Holder holder = (Holder)entry.getKey();
                List list2 = (List)entry.getValue();
                return Streams.mapWithIndex(Lists.partition((List)list2, (int)n).stream(), (list, l) -> GameTestBatchFactory.toGameTestBatch(List.copyOf(list), holder, (int)l));
            }).toList();
        };
    }

    public static GameTestBatch toGameTestBatch(Collection<GameTestInfo> collection, Holder<TestEnvironmentDefinition> holder, int n) {
        return new GameTestBatch(n, collection, holder);
    }
