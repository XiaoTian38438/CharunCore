...
implements Codec<Holder<E>> {
    ...
    public <T> DataResult<T> encode(Holder<E> holder, DynamicOps<T> dynamicOps, T t) {
        ...
        if (dynamicOps instanceof RegistryOps && (optional = (registryOps = (RegistryOps)dynamicOps).owner(this.registryKey)).isPresent()) {
            ...
            return holder.unwrap().map(resourceKey -> Identifier.CODEC.encode(resourceKey.identifier(), dynamicOps, t), object -> DataResult.error(() -> "Elements from registry " + String.valueOf(this.registryKey) + " can't be serialized to a value"));
        }
    ...
    public <T> DataResult<Pair<Holder<E>, T>> decode(DynamicOps<T> dynamicOps, T t) {
        ...
        if (dynamicOps instanceof RegistryOps && (optional = (registryOps = (RegistryOps)dynamicOps).getter(this.registryKey)).isPresent()) {
            return Identifier.CODEC.decode(dynamicOps, t).flatMap((? super R pair) -> {
                Identifier identifier = (Identifier)pair.getFirst();
                return ((HolderGetter)optional.get()).get(ResourceKey.create(this.registryKey, identifier)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Failed to get element " + String.valueOf(identifier))).map((? super R reference) -> Pair.of(reference, pair.getSecond())).setLifecycle(Lifecycle.stable());
            });
        }
        return DataResult.error(() -> "Can't access registry " + String.valueOf(this.registryKey));
    }
