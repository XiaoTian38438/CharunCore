...
public record SoundEvent(Identifier location, Optional<Float> fixedRange) {
    public static final Codec<SoundEvent> DIRECT_CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Identifier.CODEC.fieldOf("sound_id")).forGetter(SoundEvent::location), Codec.FLOAT.lenientOptionalFieldOf("range").forGetter(SoundEvent::fixedRange)).apply((Applicative<SoundEvent, ?>)instance, SoundEvent::create));
    ...
    private static SoundEvent create(Identifier identifier, Optional<Float> optional) {
        return optional.map(f -> SoundEvent.createFixedRangeEvent(identifier, f.floatValue())).orElseGet(() -> SoundEvent.createVariableRangeEvent(identifier));
    }
