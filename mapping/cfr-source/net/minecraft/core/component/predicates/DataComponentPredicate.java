...
public interface DataComponentPredicate {
    ...
    public static interface Type<T extends DataComponentPredicate> {
        ...
        private static <T extends Type<?>> Either<T, DataComponentType<?>> unpackType(T t) {
            Either<Object, Object> either;
            if (t instanceof AnyValueType) {
                AnyValueType anyValueType = (AnyValueType)t;
        ...
        private static Type<?> copyOrCreateType(Either<Type<?>, DataComponentType<?>> either) {
            return either.map(type -> type, AnyValueType::create);
        }
    ...
    public record Single<T extends DataComponentPredicate>(Type<T> type, T predicate) {
        static <T extends DataComponentPredicate> MapCodec<Single<T>> wrapCodec(Type<T> type, Codec<T> codec) {
            return RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)codec.fieldOf("value")).forGetter(Single::predicate)).apply((Applicative<Single, ?>)instance, dataComponentPredicate -> new Single<DataComponentPredicate>(type, (DataComponentPredicate)dataComponentPredicate)));
        }
