...
IdMap<T> {
    ...
    private Codec<Holder.Reference<T>> referenceHolderWithLifecycle() {
        Codec<Holder.Reference> codec = Identifier.CODEC.comapFlatMap(identifier -> this.get((Identifier)identifier).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown registry key in " + String.valueOf(this.key()) + ": " + String.valueOf(identifier))), reference -> reference.key().identifier());
    ...
    private DataResult<Holder.Reference<T>> safeCastToReference(Holder<T> holder) {
        DataResult<Holder.Reference<T>> dataResult;
    ...
    public static <T> T register(Registry<? super T> registry, String string, T t) {
        return Registry.register(registry, Identifier.parse(string), t);
    }

    public static <V, T extends V> T register(Registry<V> registry, Identifier identifier, T t) {
        return Registry.register(registry, ResourceKey.create(registry.key(), identifier), t);
    }
    ...
    public static <R, T extends R> Holder.Reference<T> registerForHolder(Registry<R> registry, ResourceKey<R> resourceKey, T t) {
        return ((WritableRegistry)registry).register(resourceKey, t, RegistrationInfo.BUILT_IN);
    }

    public static <R, T extends R> Holder.Reference<T> registerForHolder(Registry<R> registry, Identifier identifier, T t) {
        return Registry.registerForHolder(registry, ResourceKey.create(registry.key(), identifier), t);
    }
    ...
    default public IdMap<Holder<T>> asHolderIdMap() {
        return new IdMap<Holder<T>>(){
            ...
            @Override
            public @Nullable Holder<T> byId(int n) {
