...
public class Gizmos {
    ...
    public static GizmoProperties line(Vec3 vec3, Vec3 vec32, int n) {
        return Gizmos.addGizmo(new LineGizmo(vec3, vec32, n, 3.0f));
    }

    public static GizmoProperties line(Vec3 vec3, Vec3 vec32, int n, float f) {
        return Gizmos.addGizmo(new LineGizmo(vec3, vec32, n, f));
    }

    public static GizmoProperties arrow(Vec3 vec3, Vec3 vec32, int n) {
        return Gizmos.addGizmo(new ArrowGizmo(vec3, vec32, n, 2.5f));
    }

    public static GizmoProperties arrow(Vec3 vec3, Vec3 vec32, int n, float f) {
        return Gizmos.addGizmo(new ArrowGizmo(vec3, vec32, n, f));
    }
    ...
    public static GizmoProperties point(Vec3 vec3, int n, float f) {
        return Gizmos.addGizmo(new PointGizmo(vec3, n, f));
    }

    public static GizmoProperties billboardTextOverBlock(String string, BlockPos blockPos, int n, int n2, float f) {
        double d = 1.3;
        double d2 = 0.2;
        GizmoProperties gizmoProperties = Gizmos.billboardText(string, Vec3.atLowerCornerWithOffset(blockPos, 0.5, 1.3 + (double)n * 0.2, 0.5), TextGizmo.Style.forColorAndCentered(n2).withScale(f));
        gizmoProperties.setAlwaysOnTop();
        return gizmoProperties;
    }

    public static GizmoProperties billboardTextOverMob(Entity entity, int n, String string, int n2, float f) {
        double d = 2.4;
        double d2 = 0.25;
        double d3 = (double)entity.getBlockX() + 0.5;
        double d4 = entity.getY() + 2.4 + (double)n * 0.25;
        double d5 = (double)entity.getBlockZ() + 0.5;
        float f2 = 0.5f;
        GizmoProperties gizmoProperties = Gizmos.billboardText(string, new Vec3(d3, d4, d5), TextGizmo.Style.forColor(n2).withScale(f).withLeftAlignment(0.5f));
        gizmoProperties.setAlwaysOnTop();
        return gizmoProperties;
    }
