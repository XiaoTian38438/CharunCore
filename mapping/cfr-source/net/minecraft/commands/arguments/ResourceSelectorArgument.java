...
implements ArgumentType<Collection<Holder.Reference<T>>> {
    ...
    public Collection<Holder.Reference<T>> parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        List<Holder.Reference<T>> list = this.registryLookup.listElements().filter(reference -> ResourceSelectorArgument.matches(string, reference.key().identifier())).toList();
        if (list.isEmpty()) {
