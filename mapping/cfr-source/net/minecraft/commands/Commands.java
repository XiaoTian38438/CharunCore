...
public class Commands {
    ...
    private static @Nullable ContextChain<CommandSourceStack> finishParsing(ParseResults<CommandSourceStack> parseResults, String string, CommandSourceStack commandSourceStack) {
        try {
            ...
            return ContextChain.tryFlatten(parseResults.getContext().build(string)).orElseThrow(() -> CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownCommand().createWithContext(parseResults.getReader()));
        }
    ...
    public static void executeCommandInContext(CommandSourceStack commandSourceStack, Consumer<ExecutionContext<CommandSourceStack>> consumer) {
        block9: {
            ...
            if (bl) {
                ...
                int n = Math.max(1, gameRules.get(GameRules.MAX_COMMAND_SEQUENCE_LENGTH));
                int n2 = gameRules.get(GameRules.MAX_COMMAND_FORKS);
