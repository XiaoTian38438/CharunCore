...
public class SpawnUtil {
    public static <T extends Mob> Optional<T> trySpawnMob(EntityType<T> entityType, EntitySpawnReason entitySpawnReason, ServerLevel serverLevel, BlockPos blockPos, int n, int n2, int n3, Strategy strategy, boolean bl) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (!serverLevel.getWorldBorder().isWithinBounds(mutableBlockPos) || !SpawnUtil.moveToPossibleSpawnPosition(serverLevel, n3, mutableBlockPos, strategy) || bl && !serverLevel.noCollision(entityType.getSpawnAABB((double)mutableBlockPos.getX() + 0.5, mutableBlockPos.getY(), (double)mutableBlockPos.getZ() + 0.5)) || (mob = (Mob)entityType.create(serverLevel, null, mutableBlockPos, entitySpawnReason, false, false)) == null) continue;
