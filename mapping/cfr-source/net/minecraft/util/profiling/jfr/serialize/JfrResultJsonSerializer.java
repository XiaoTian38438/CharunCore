...
public class JfrResultJsonSerializer {
    ...
    private JsonElement chunkGen(List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> list) {
        ...
        for (Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>> pair2 : list) {
            ...
            jsonObject3.addProperty("state", pair2.getFirst().toString());
            ...
            jsonObject3.add("fastest", function.apply(timedStatSummary.fastest()));
            jsonObject3.add("slowest", function.apply(timedStatSummary.slowest()));
            jsonObject3.add("secondSlowest", (JsonElement)(timedStatSummary.secondSlowest() != null ? function.apply(timedStatSummary.secondSlowest()) : JsonNull.INSTANCE));
        }
        return jsonObject;
    }
    ...
    private JsonElement fps(List<FpsStat> list) {
        ...
        JsonObject jsonObject = new JsonObject();
        int[] nArray = list.stream().mapToInt(FpsStat::fps).toArray();
        ...
        Map<Integer, Double> map = Percentiles.evaluate(nArray);
        map.forEach((n, d) -> jsonObject.addProperty("p" + n, (Number)d));
        return jsonObject;
    }
    ...
    private <T> JsonElement ioSummary(IoSummary<T> ioSummary, BiConsumer<T, JsonObject> biConsumer) {
        ...
        ioSummary.largestSizeContributors().forEach(pair -> {
            ...
            biConsumer.accept(f, jsonObject);
