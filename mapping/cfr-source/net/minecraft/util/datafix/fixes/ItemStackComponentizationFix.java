...
extends DataFix {
    ...
    private static void fixItemStack(ItemStackData itemStackData, Dynamic<?> dynamic3) {
        ...
        if (itemStackData.removeTag("Unbreakable").asBoolean(false)) {
            ...
            if ((n & 4) != 0) {
                object2 = ((Dynamic)object2).set("show_in_tooltip", dynamic3.createBoolean(false));
            }
        ...
        if (((Optional)object2).isPresent()) {
            ...
            if ((n & 0x80) != 0) {
                object = ((Dynamic)object).set("show_in_tooltip", ((DynamicLike)object).createBoolean(false));
            }
        ...
        if (itemStackData.is("minecraft:filled_map")) {
            ...
            if (!object.isEmpty()) {
                itemStackData.setComponent("minecraft:map_decorations", dynamic3.createMap((Map<Dynamic<?>, Dynamic<?>>)object));
            }
        }
    ...
    private static Dynamic<?> fixDisplay(ItemStackData itemStackData, Dynamic<?> dynamic2, int n) {
        Dynamic dynamic3;
        boolean bl;
        ...
        if (optional.isPresent() || bl) {
            dynamic3 = dynamic2.emptyMap().set("rgb", dynamic2.createInt(optional.orElse(10511680)));
            if (bl) {
                dynamic3 = dynamic3.set("show_in_tooltip", dynamic2.createBoolean(false));
            }
            itemStackData.setComponent("minecraft:dyed_color", dynamic3);
        }
        if (((Optional)((Object)(dynamic3 = dynamic2.get("LocName").asString().result()))).isPresent()) {
            itemStackData.setComponent("minecraft:item_name", LegacyComponentDataFixUtils.createTranslatableComponent(dynamic2.getOps(), (String)((Optional)((Object)dynamic3)).get()));
        }
        if (itemStackData.is("minecraft:filled_map")) {
    ...
    private static <T> Dynamic<T> fixBlockEntityTag(ItemStackData itemStackData, Dynamic<T> dynamic2, String string) {
        ...
        Optional<Dynamic<T>> optional = dynamic2.get("LootTable").result();
        if (optional.isPresent()) {
            Dynamic dynamic3 = dynamic2.emptyMap().set("loot_table", optional.get());
            long l = dynamic2.get("LootTableSeed").asLong(0L);
            if (l != 0L) {
                dynamic3 = dynamic3.set("seed", dynamic2.createLong(l));
            }
            itemStackData.setComponent("minecraft:container_loot", dynamic3);
        ...
        return switch (string) {
            ...
            case "minecraft:decorated_pot" -> {
                ...
                Optional<Dynamic<T>> var6_7 = dynamic2.get("item").result();
                if (var6_7.isPresent()) {
                    itemStackData.setComponent("minecraft:container", dynamic2.createList(Stream.of(dynamic2.emptyMap().set("slot", dynamic2.createInt(0)).set("item", var6_7.get()))));
                }
                yield dynamic2.remove("sherds").remove("item");
            }
            case "minecraft:banner" -> {
                itemStackData.setComponent("minecraft:banner_patterns", dynamic2.get("patterns"));
                Optional<Number> var6_8 = dynamic2.get("Base").asNumber().result();
                if (var6_8.isPresent()) {
                    itemStackData.setComponent("minecraft:base_color", dynamic2.createString(ExtraDataFixUtils.dyeColorIdToName(var6_8.get().intValue())));
                }
            ...
            case "minecraft:shulker_box", "minecraft:chest", "minecraft:trapped_chest", "minecraft:furnace", "minecraft:ender_chest", "minecraft:dispenser", "minecraft:dropper", "minecraft:brewing_stand", "minecraft:hopper", "minecraft:barrel", "minecraft:smoker", "minecraft:blast_furnace", "minecraft:campfire", "minecraft:chiseled_bookshelf", "minecraft:crafter" -> {
                List<Dynamic> var6_9 = dynamic2.get("Items").asList(dynamic -> dynamic.emptyMap().set("slot", dynamic.createInt(dynamic.get("Slot").asByte((byte)0) & 0xFF)).set("item", dynamic.remove("Slot")));
                if (!var6_9.isEmpty()) {
    ...
    private static void fixEnchantments(ItemStackData itemStackData, Dynamic<?> dynamic2, String string, String string2, boolean bl) {
        ...
        List<Pair> list = optionalDynamic.asList(Function.identity()).stream().flatMap(dynamic -> ItemStackComponentizationFix.parseEnchantment(dynamic).stream()).filter(pair -> (Integer)pair.getSecond() > 0).toList();
        if (!list.isEmpty() || bl) {
            Dynamic dynamic3 = dynamic2.emptyMap();
            Dynamic dynamic4 = dynamic2.emptyMap();
            for (Pair pair2 : list) {
                dynamic4 = dynamic4.set((String)pair2.getFirst(), dynamic2.createInt((Integer)pair2.getSecond()));
            }
    ...
    private static Optional<Pair<String, Integer>> parseEnchantment(Dynamic<?> dynamic) {
        return dynamic.get("id").asString().apply2stable((string, number) -> Pair.of(string, Mth.clamp(number.intValue(), 0, 255)), dynamic.get("lvl").asNumber()).result();
    }
    ...
    private static void fixBlockStatePredicates(ItemStackData itemStackData, Dynamic<?> dynamic2, String string, String string2, boolean bl) {
        Optional<Dynamic<?>> optional = itemStackData.removeTag(string).result();
        if (optional.isEmpty()) {
            return;
        }
        Dynamic dynamic3 = dynamic2.emptyMap().set("predicates", dynamic2.createList(optional.get().asStream().map(dynamic -> DataFixUtils.orElse(dynamic.asString().map(string -> ItemStackComponentizationFix.fixBlockStatePredicate(dynamic, string)).result(), dynamic))));
        if (bl) {
            dynamic3 = dynamic3.set("show_in_tooltip", dynamic2.createBoolean(false));
        }
        itemStackData.setComponent(string2, dynamic3);
    ...
    private static Dynamic<?> fixAttributeModifier(Dynamic<?> dynamic2) {
        Dynamic<Object> dynamic3 = dynamic2.emptyMap().set("name", dynamic2.createString("")).set("amount", dynamic2.createDouble(0.0)).set("operation", dynamic2.createString("add_value"));
        dynamic3 = Dynamic.copyField(dynamic2, "AttributeName", dynamic3, "type");
        dynamic3 = Dynamic.copyField(dynamic2, "Slot", dynamic3, "slot");
        dynamic3 = Dynamic.copyField(dynamic2, "UUID", dynamic3, "uuid");
        dynamic3 = Dynamic.copyField(dynamic2, "Name", dynamic3, "name");
        dynamic3 = Dynamic.copyField(dynamic2, "Amount", dynamic3, "amount");
        dynamic3 = Dynamic.copyAndFixField(dynamic2, "Operation", dynamic3, "operation", dynamic -> dynamic.createString(switch (dynamic.asInt(0)) {
            default -> "add_value";
    ...
    private static void fixPotionContents(ItemStackData itemStackData, Dynamic<?> dynamic) {
        ...
        if (!(dynamic2 = itemStackData.moveTagInto("custom_potion_effects", dynamic2, "custom_effects")).equals(dynamic.emptyMap())) {
    ...
    private static void fixLodestoneTracker(ItemStackData itemStackData, Dynamic<?> dynamic) {
        Optional<Dynamic<?>> optional = itemStackData.removeTag("LodestonePos").result();
        Optional<Dynamic<?>> optional2 = itemStackData.removeTag("LodestoneDimension").result();
    ...
    public static Dynamic<?> fixProfile(Dynamic<?> dynamic) {
        ...
        if (optional.isPresent()) {
            if (ItemStackComponentizationFix.isValidPlayerName(optional.get())) {
    ...
    static class ItemStackData {
        ...
        public static Optional<ItemStackData> read(Dynamic<?> dynamic) {
            return dynamic.get("id").asString().apply2stable((string, number) -> new ItemStackData((String)string, number.intValue(), dynamic.remove("id").remove("Count")), dynamic.get("Count").asNumber()).result();
        }
        ...
        public void setComponent(String string, OptionalDynamic<?> optionalDynamic) {
            optionalDynamic.result().ifPresent(dynamic -> {
                this.components = this.components.set(string, (Dynamic<?>)dynamic);
            });
        }

        public Dynamic<?> moveTagInto(String string, Dynamic<?> dynamic, String string2) {
            Optional<Dynamic<?>> optional = this.removeTag(string).result();
            if (optional.isPresent()) {
                return dynamic.set(string2, optional.get());
            }
            return dynamic;
        }
        ...
        public void moveTagToComponent(String string, String string2, Dynamic<?> dynamic) {
            Optional<Dynamic<?>> optional = this.removeTag(string).result();
            if (optional.isPresent() && !optional.get().equals(dynamic)) {
                this.setComponent(string2, optional.get());
            }
        }
        ...
        public void fixSubTag(String string, boolean bl, UnaryOperator<Dynamic<?>> unaryOperator) {
            ...
            this.tag = (dynamic = (Dynamic)unaryOperator.apply(dynamic)).equals(dynamic.emptyMap()) ? this.tag.remove(string) : this.tag.set(string, dynamic);
        }
        ...
        private static <T> Dynamic<T> mergeRemainder(Dynamic<T> dynamic, Dynamic<?> dynamic2) {
            ...
            return dynamicOps.getMap(dynamic.getValue()).flatMap(mapLike -> dynamicOps.mergeToMap((Object)dynamic2.convert(dynamicOps).getValue(), (MapLike)mapLike)).map(object -> new Dynamic<Object>(dynamicOps, object)).result().orElse(dynamic);
        }
