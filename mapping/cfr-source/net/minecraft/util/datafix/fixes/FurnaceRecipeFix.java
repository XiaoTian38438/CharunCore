        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (n2 > 0) {
                optional.ifPresent(dynamic -> {
                    ...
                    optional.ifPresent(pair -> arrayList.add(Pair.of(pair.getFirst(), n2)));
                });
            }
            dynamic2 = dynamic2.remove(string).remove(string2);
        }
        return typed.set(DSL.remainderFinder(), type2, Pair.of(Either.left(Pair.of(arrayList, dynamic2.emptyMap())), dynamic2));
    }
}
