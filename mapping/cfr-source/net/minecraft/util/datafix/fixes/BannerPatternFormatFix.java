...
extends NamedEntityFix {
    ...
    private static Dynamic<?> fixLayer(Dynamic<?> dynamic2) {
        dynamic2 = dynamic2.renameAndFixField("Pattern", "pattern", dynamic -> DataFixUtils.orElse(dynamic.asString().map(string -> PATTERN_ID_MAP.getOrDefault(string, (String)string)).map(dynamic::createString).result(), dynamic));
