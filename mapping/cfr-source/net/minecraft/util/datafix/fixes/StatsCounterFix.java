...
extends DataFix {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", type, type2, (Typed<?> typed) -> {
            ...
            if (optional.isPresent()) {
                for (Map.Entry<Dynamic<?>, Dynamic<?>> entry : optional.get().entrySet()) {
                    String string;
                    StatType statType;
                    if (!entry.getValue().asNumber().result().isPresent() || (statType = StatsCounterFix.unpackLegacyKey(string = entry.getKey().asString(""))) == null) continue;
                    ...
                    hashMap.put(dynamic3, dynamic4.set(statType.typeKey(), entry.getValue()));
                }
            }
            return Util.readTypedOrThrow(type2, dynamic.emptyMap().set("stats", dynamic.createMap(hashMap)));
        });
    }
    ...
    private TypeRewriteRule makeObjectiveFixer() {
        ...
        return this.fixTypeEverywhereTyped("ObjectiveStatFix", type, type2, (Typed<?> typed) -> {
            ...
            Dynamic<?> dynamic3 = dynamic2.update("CriteriaName", dynamic -> DataFixUtils.orElse(dynamic.asString().result().map(string -> {
