...
extends DataFix {
    ...
    private static Dynamic<?> fixItemStackComponents(Dynamic<?> dynamic) {
        return dynamic.update("minecraft:attribute_modifiers", dynamic2 -> dynamic2.update("modifiers", dynamic -> DataFixUtils.orElse(dynamic.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(dynamic::createList), dynamic)));
    }

    private static Dynamic<?> fixAttribute(Dynamic<?> dynamic) {
        return dynamic.renameField("Name", "id").renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", dynamic2 -> DataFixUtils.orElse(dynamic2.asStreamOpt().result().map(stream -> stream.map(AttributeModifierIdFix::convertModifierForEntity)).map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(dynamic::createList), dynamic2));
    }

    private static Typed<?> fixEntity(Typed<?> typed) {
        return typed.update(DSL.remainderFinder(), dynamic2 -> dynamic2.renameAndFixField("Attributes", "attributes", dynamic -> DataFixUtils.orElse(dynamic.asStreamOpt().result().map(stream -> stream.map(AttributeModifierIdFix::fixAttribute)).map(dynamic::createList), dynamic)));
    }

    public static @Nullable UUID uuidFromIntArray(int[] nArray) {
        if (nArray.length != 4) {
            return null;
        }
