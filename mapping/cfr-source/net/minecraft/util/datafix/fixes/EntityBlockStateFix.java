...
extends DataFix {
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        ...
        return typed.update(type.finder(), type2, either -> {
            int n = either.map(pair -> ((Either)pair.getSecond()).map(n -> n, EntityBlockStateFix::getBlockId), unit -> {
            ...
            return Either.left(Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(n << 4 | n2)));
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String string, String string2, String string3) {
        Tag.TagType<Pair<String, Either<Integer, String>>> tagType = DSL.field(string, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
        Tag.TagType<Pair<String, Dynamic<?>>> tagType2 = DSL.field(string3, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
        Dynamic<?> dynamic = typed.getOrCreate(DSL.remainderFinder());
        return typed.update(tagType.finder(), tagType2, pair -> {
            int n2 = ((Either)pair.getSecond()).map(n -> n, EntityBlockStateFix::getBlockId);
            int n3 = dynamic.get(string2).asInt(0) & 0xF;
            return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(n2 << 4 | n3));
