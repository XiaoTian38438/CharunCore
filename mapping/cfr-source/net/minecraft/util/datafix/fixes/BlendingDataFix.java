...
extends DataFix {
    ...
    private static Dynamic<?> updateChunkTag(Dynamic<?> dynamic, OptionalDynamic<?> optionalDynamic) {
        ...
        if (bl && optional.isPresent()) {
            Dynamic<?> dynamic2;
            String string;
            String string2 = NamespacedSchema.ensureNamespaced(optional.get().asString("empty"));
            ...
            } else if (optional2.isPresent() && !STATUSES_TO_SKIP_BLENDING.contains(string = NamespacedSchema.ensureNamespaced((dynamic2 = optional2.get()).get("target_status").asString("empty")))) {
