...
extends DataFix {
    ...
    private <T> Dynamic<T> updateItem(Dynamic<T> dynamic, String string) {
        ...
        if (n == -1) {
            ...
            Dynamic dynamic3 = ParticleUnflatteningFix.parseTag(dynamic.getOps(), string.substring(n));
            if (dynamic3 != null) {
    ...
    private static <T> @Nullable Dynamic<T> parseTag(DynamicOps<T> dynamicOps, String string) {
        try {
            return new Dynamic<T>(dynamicOps, TagParser.create(dynamicOps).parseFully(string));
        }
    ...
    private static <T> Dynamic<T> readVector(Dynamic<T> dynamic, StringReader stringReader) throws CommandSyntaxException {
        ...
        return dynamic.createList(Stream.of(Float.valueOf(f), Float.valueOf(f2), Float.valueOf(f3)).map(dynamic::createFloat));
    }
