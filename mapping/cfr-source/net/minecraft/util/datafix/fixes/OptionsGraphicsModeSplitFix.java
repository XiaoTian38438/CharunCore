...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("graphicsMode split to " + this.newFieldName, this.getInputSchema().getType(References.OPTIONS), typed -> typed.update(DSL.remainderFinder(), dynamic -> DataFixUtils.orElseGet(dynamic.get("graphicsMode").asString().map(string -> dynamic.set(this.newFieldName, dynamic.createString(this.getValue((String)string)))).result(), () -> dynamic.set(this.newFieldName, dynamic.createString(this.valueIfFancy)))));
    }
