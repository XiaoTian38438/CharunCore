...
extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> dynamic2) {
        Optional<Dynamic> optional = dynamic2.get("ench").asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(dynamic2::createList).result();
        if (optional.isPresent()) {
            dynamic2 = dynamic2.remove("ench").set("Enchantments", optional.get());
        }
        return dynamic2.update("StoredEnchantments", dynamic -> DataFixUtils.orElse(dynamic.asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)MAP.getOrDefault(dynamic.get("id").asInt(0), (Object)"null"))))).map(dynamic::createList).result(), dynamic));
    }
}
