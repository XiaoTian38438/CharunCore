        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhere("EntityRidingToPassengerFix", type4, type5, dynamicOps -> pair2 -> {
            ...
            while (true) {
                Either either = DataFixUtils.orElse(optional.map(pair -> {
                    ...
                    Object FT = typed.set(opticFinder2, pair).getOptional(opticFinder3).orElseThrow(() -> new IllegalStateException("Should always have an entity tree here"));
                    return Either.left(ImmutableList.of(FT));
                }), Either.right(DSL.unit()));
                optional = Optional.of(Pair.of(References.ENTITY_TREE.typeName(), Pair.of(either, ((Pair)pair3.getSecond()).getSecond())));
                Optional optional2 = ((Either)((Pair)pair3.getSecond()).getFirst()).left();
                if (optional2.isEmpty()) break;
                pair3 = (Pair)new Typed(type, (DynamicOps<?>)dynamicOps, optional2.get()).getOptional(opticFinder).orElseThrow(() -> new IllegalStateException("Should always have an entity here"));
            }
