...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("InlineBlockPosFormatFix - player", this.getInputSchema().getType(References.PLAYER), typed -> typed.update(DSL.remainderFinder(), this::fixPlayer)), this.fixTypeEverywhereTyped("InlineBlockPosFormatFix - entity", this.getInputSchema().getType(References.ENTITY), typed2 -> {
    ...
    private Dynamic<?> fixPlayer(Dynamic<?> dynamic) {
        ...
        if (optional.isPresent() && optional2.isPresent() && optional3.isPresent()) {
            dynamic2 = dynamic.createMap(Map.of(dynamic.createString("pos"), ExtraDataFixUtils.createBlockPos(dynamic, optional.get().intValue(), optional2.get().intValue(), optional3.get().intValue())));
        ...
        if (((Optional)((Object)(dynamic2 = dynamic.get("enteredNetherPosition").result()))).isPresent()) {
            dynamic = dynamic.remove("enteredNetherPosition").set("entered_nether_pos", dynamic.createList(Stream.of(dynamic.createDouble(((Optional)((Object)dynamic2)).get().get("x").asDouble(0.0)), dynamic.createDouble(((Dynamic)((Optional)((Object)dynamic2)).get()).get("y").asDouble(0.0)), dynamic.createDouble(((Dynamic)((Optional)((Object)dynamic2)).get()).get("z").asDouble(0.0)))));
        }
        return dynamic;
    }
