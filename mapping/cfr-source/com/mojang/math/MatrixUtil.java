...
public class MatrixUtil {
    ...
    private static boolean checkPropertyRaw(Matrix4fc matrix4fc, int n) {
        return (matrix4fc.properties() & n) != 0;
    }

    public static boolean checkProperty(Matrix4fc matrix4fc, int n) {
        if (MatrixUtil.checkPropertyRaw(matrix4fc, n)) {
            return true;
        }
        if (matrix4fc instanceof Matrix4f) {
            Matrix4f matrix4f = (Matrix4f)matrix4fc;
            matrix4f.determineProperties();
            return MatrixUtil.checkPropertyRaw(matrix4fc, n);
        }
        return false;
    }
