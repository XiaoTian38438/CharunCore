...
public abstract class ArgumentBuilder<S, T extends ArgumentBuilder<S, T>> {
    ...
    private Predicate<S> requirement = object -> true;
    ...
    public T redirect(CommandNode<S> commandNode) {
        return this.forward(commandNode, null, false);
    }

    public T redirect(CommandNode<S> commandNode, SingleRedirectModifier<S> singleRedirectModifier) {
        return this.forward(commandNode, singleRedirectModifier == null ? null : commandContext -> Collections.singleton(singleRedirectModifier.apply(commandContext)), false);
    }

    public T fork(CommandNode<S> commandNode, RedirectModifier<S> redirectModifier) {
        return this.forward(commandNode, redirectModifier, true);
    }
